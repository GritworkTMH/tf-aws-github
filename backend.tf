resource "aws_s3_bucket" "bca_aws_master_tfstate" {
  bucket = var.bca_aws_master_tfstate
}

resource "aws_s3_bucket_ownership_controls" "bca_aws_master_s3_ownership" {
...
resource "aws_s3_bucket_acl" "s3_bucket_acl_tfstate" {
  depends_on = [aws_s3_bucket_ownership_controls.bca_aws_master_s3_ownership]
  bucket     = aws_s3_bucket.bca_aws_master_tfstate.id
  acl        = "private"
}
